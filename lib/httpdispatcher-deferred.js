/** 
 * Fork of https://github.com/alberto-bottarini/httpdispatcher.git v2.0.1, 
 * but adding support for deferred listener resolution (e.g. so 
 * afterFilters don't execute before the actual handler.)
 */
...
var HttpDispatcher = function() {
	...
	this.fatal = function(err, req, res) {
		console.error("Request to:", req.url, 'threw', err.name, 'with message:', err.message);
		res.writeHead(500);
...
HttpDispatcher.prototype.onFatal = function(cb) {
    this.fatal = cb;
};
...
HttpDispatcher.prototype.dispatch = function(req, res) {
    ...
    var doDispatch = function() {
        ...
        httpChain.add(httpChain.getCatchWrapped(listenerCb, this.fatal));
...
HttpChain.prototype.getWrapped = function(cb) {
    return function(req, res, chain) {
		...
		if (result != null && typeof result == 'object' && result.constructor.name == 'Promise') {
			// Deferred render -|> (proceed on either success or fail)
			var request_ctn = () => { chain.next(req, res); };
			result.then(request_ctn, request_ctn);
		} else {
			// Proceed ->
        	chain.next(req, res);
		}
...
HttpChain.prototype.getCatchWrapped = function(cb, fh) {
    return function(req, res, chain) {
		try {
			var result = cb(req, res);
			if (result != null && typeof result == 'object' && result.constructor.name == 'Promise') {
